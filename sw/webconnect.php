<?php
	...
	$profit = number_format($account[AccProfit],2,".","")." ".$account[Currency]; 
	...
	if($len != 0){
		...
		while($stmt->fetch()){
			...
			if($mt == 0) continue; // market is closed
			UpdateEAsInfo($db,$_id,$_startlot,$_maxorders,$nOrders,$AvgTP,$nextLot,$diff_t,$diff_s,$PL,$account[MarketTime]);
			if($nOrders == 0){
				if($_active){
			...
			if($diff_s <= 0) {
				if($nOrders < $_maxorders){
					$cmd .= ";ordersend=".$_sym.",".$_type.",".$nextLot.",".$comm."-".$nOrders.",".$_magic;
					//SendEmail("OrderSend ".$_sym." (".($_type == OP_BUY ? " | BUY  " : " | SELL ")."), next lot: ".$nextLot .", #: " . $nOrders . ", DD: " . $PL);
					continue;  //// neu hinzugefügt
					}
				else if($_plan == 1){ // Plan 1 : set active = false und MaxOrders = 0 und email(hilfe!brauche unlocker!)
					//$cmd .= ";closeallorders=".$_magic;
					SendEmail("MaxOrders erreicht!!! (".$account[Number]."):".$_sym."_".$_type."_".$_magic."\n"."\n".$cmd . "hilfe!brauche unlocker!");
					//UpdateEA
					$sql  = "UPDATE EA ";
					$sql .= " SET MaxOrders = 0, Active = 0";
					$sql .= " WHERE ID = ?";
					if (!$stmt = $db->prepare($sql)) die("prepare set UNlocker ".$db->error."#".$sql);
					if(!$stmt->bind_param('i',$_id)) die("binding set UNlocker ".$stmt->error);
					if(!$stmt->execute()) die("execute set UNlocker ".$stmt->error);
					$stmt->free_result();
					$stmt->close();
					continue;
					}
				else if($_plan == 2){ // Plan 2 schliessen bei MaxOrders!!!!
					$cmd .= ";closeallorders=".$_magic;
					SendEmail("MaxOrders erreicht!!! (".$account[Number]."):".$account[Name]."\n".$cmd);
					continue;
					}
	  			}
	  		if($_stealth){
	...
	function UpdateEAsInfo($_db,$ID,$StartLot,$MaxOrders,$OpenOrders,$TakeProfit,$NextLot,$TPp,$NLp,$Profit,$ttime){
			$sql  = "UPDATE EA ";
			...
			$stmt->close();
			/*
			$sql = "SELECT ID,Stand FROM Drawdown where EA = ? AND Datum = current_date";
			if (!$stmt = $_db->prepare($sql)) die("prepare failed(select from drawdown) ".$_db->error);
			if(!$stmt->bind_param('i', $ID)) die("binding failed(select from drawdown) ".$stmt->error);
			if(!$stmt->execute()) die("execute failed(select from drawdown) ".$stmt->error);
			$result = $stmt->store_result();
			
			$stmt->bind_result($eid,$Stand);
			$NUM = $stmt->num_rows;
			if($NUM != 0){
				$stmt->fetch();
				if($Stand > $Profit){
					$stmt->free_result();
					$stmt->close();
					$sql  = "UPDATE Drawdown SET Stand = ? WHERE ID = " . $eid;
					if (!$stmt = $_db->prepare($sql)) die("prepare failed(update drawdown) ".$_db->error."#".$sql);
					if (!$stmt->bind_param('d',$Profit)) die("binding failed(update drawdown) ".$stmt->error);
					if(!$stmt->execute()) die("execute failed(update drawdown) ".$stmt->error);
				}
			}else{
				$stmt->free_result();
				$stmt->close();
					$sql  = "INSERT INTO Drawdown (EA,Datum,Stand) VALUES (?,?,?)";
					if (!$stmt = $_db->prepare($sql)) die("prepare failed(insert into drawdown) ".$_db->error."#".$sql);
					if(!$stmt->bind_param('isd',$ID, $ttime, $Profit)) die("binding failed(insert into drawdown) ".$stmt->error);
					if(!$stmt->execute()) die("execute failed(insert into drawdown) ".$stmt->error);
			}
			$stmt->free_result();
			$stmt->close();
			*/
	...
	function GetPostData($db){
    	...
    	if($account[Number] != "" && $account[Name] != "" && $account[Broker] != ""){
			...
			$sql = "SELECT ID,MarketTime,Updated,Coins FROM Account where Number = ? AND Broker = ?";
			...
			$ID = $stmt->num_rows;
			if($ID != 0){
				$stmt->bind_result($ID,$mt,$updated,$coins);
				$stmt->fetch();
				$stmt->free_result();
				$stmt->close();
				...
				$sql  = "UPDATE Account ";
				$sql .= " SET Leverage = ?, Balance = ?,Equity = ?,Profit = ?,MarketTime = ?,MarketOpen = ?,Email = ?, Credit = ?, FreeMargin = ?, Updated = now()";
				$sql .= " WHERE ID = ?";
				if (!$stmt = $db->prepare($sql)) die("prepare failed(2) ".$db->error."#".$sql);
				if(!$stmt->bind_param('idddsisddi',$account[Leverage],$account[Balance], $account[Equity], $account[AccProfit], $account[MarketTime], $mt,$account[Email],$account[AccCredit],$account[AccFreeMargin], $ID)) die("binding failed(2) ".$stmt->error);
				if(!$stmt->execute()) die("execute failed(2) ".$stmt->error);			
				}
			else{  // no account - insert new!
				$stmt->free_result();
				$stmt->close();
				$pf = "";
				$mt = 1;
				...
				*/
				...
				$sql  = "INSERT INTO EA (ID,Account,Symbol,Type) VALUES (?,?,?,?)";
				if (!$stmt = $db->prepare($sql)) die("prepare failed(16) ".$db->error."#".$sql);
				if(!$stmt->bind_param('iiii',$myID,$ID,$sym,$type)) die("binding failed(16) ".$stmt->error);
				$sym = 0;$type = 0;$myID = $ID*1000 + $sym*100 + $type*10;
				if(!$stmt->execute()) die("execute failed(1.6) ".$stmt->error);
				$sym = 0;$type = 1;$myID = $ID*1000 + $sym*100 + $type*10;
				if(!$stmt->execute()) die("execute failed(2.6) ".$stmt->error);
				$sym = 2;$type = 0;$myID = $ID*1000 + $sym*100 + $type*10;
				if(!$stmt->execute()) die("execute failed(1.6) ".$stmt->error);
				$sym = 2;$type = 1;$myID = $ID*1000 + $sym*100 + $type*10;
				if(!$stmt->execute()) die("execute failed(2.6) ".$stmt->error);
			...
			$stmt->free_result();
			$stmt->close();
			$sql = "SELECT Stand, Equity, Coins FROM Balance where ID = (SELECT MAX(ID) FROM Balance WHERE Account = ?)";
			...
			//$result = $stmt->store_result();
			//$NUM = $stmt->num_rows;
			$stmt->bind_result($MAX_St,$MAX_Eq,$MAX_Co);
			$stmt->fetch();
			if ($coins == 0) $coins = 100000;
			if ($MAX_Co == 0) $MAX_Co = 100000;
			if($MAX_St != $account[Balance] || round($account[Equity] / $MAX_Co, 2) != round($MAX_Eq / $MAX_Co, 2)){
				$stmt->free_result();
				$stmt->close();
				$sql  = "INSERT INTO Balance (Account,Datum,Stand,Equity,Coins) VALUES (?,?,?,?,?)";
				if (!$stmt = $db->prepare($sql)) die("prepare failed(6) ".$db->error."#".$sql);
				if(!$stmt->bind_param('isddi',$ID, $account[MarketTime], $account[Balance], $account[Equity], $MAX_Co)) die("binding failed(6) ".$stmt->error);
	...
	function SendEmail($emailbody){
		global $account;
		$to = 'antonsander@yandex.com';
		$email_subject = "sanderinvest #".$account[Number];
